...
#'                   annot = annot,
#'                   outdir = NULL)
indelSNP <- function(nucmer = nucmer, saveRda = FALSE, refseq = refseq, gff3 = gff3, annot = annot, outdir = NULL){
  if(is.null(outdir) == FALSE){
    ...
    results$refpos <- as.numeric(results$refpos)
    
    results$pro_variant <- str_c(results$protein, results$variant, sep = ":")
    results$ID<- vapply(strsplit(as.character(results$sample), "[|]"), function(x) x[2], character(1))
    return(results)
